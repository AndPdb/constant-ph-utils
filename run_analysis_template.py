...
def main():
    ...
    for path in PATHS_MD:
        if not os.path.exists(path):
            raise FileNotFoundError(f"Directory {path} does not exist. Please check the path and try again.")
    ...
    lambda_hist_md1 = plot_lambda_hist(PATH_MD1, xvg_data_list[0], coord2lambda_dict, lambda_ref, quality=PLOT_TYPE)
    lambda_hist_md1.savefig(f"{MD1_PREFIX}_histograms.png")
    lambda_hist_md1.close()
    

    ## Protonation fraction time series
    proton_ts_md1 = plot_protonation_timeseries(PATH_MD1, time_MD1, xvg_data_list[0], coord2lambda_dict, lambda_ref, quality=PLOT_TYPE, npz_output=NPZ_OUTPUT)
    ...
    if RUN_TYPE == "Publication":
        ...
        # ## Overview lambda distributions
        lambda_hist_md2 = plot_lambda_hist(PATH_MD2, xvg_data_list[1], coord2lambda_dict, lambda_ref, quality=PLOT_TYPE)
        lambda_hist_md2.savefig(f"{MD2_PREFIX}_histograms.png")
        lambda_hist_md2.close()

        # ## Protonation fraction time series
        proton_ts_md2 = plot_protonation_timeseries(PATH_MD2, time_MDs, xvg_data_list[1], coord2lambda_dict, lambda_ref, quality=PLOT_TYPE, npz_output=NPZ_OUTPUT)
        ...
        proton_conv = plot_protonation_convergence(PATHS_MD, time_MDs, xvg_data_list, coord2lambda_dict, lambda_ref, quality=PLOT_TYPE)
        proton_conv.savefig(f"{MD1_PREFIX}_{MD2_PREFIX}_convergence.png")
        proton_conv.close()

        # ## Overview protonation fractions
        proton_frac = plot_protonation_fraction(xvg_data_list, lambda_ref, npz_output=NPZ_OUTPUT)
        ...
        res1_conv = single_residue_convergence(glu75_id, xvg_data_list, lambda_ref)
        res1_conv.savefig("Glu75.png")
        res1_conv.close()
        
        # ### Glu78
        glu78_id = resid2coordid(78, lambda_ref)
        res2_conv = single_residue_convergence(glu78_id, xvg_data_list, lambda_ref)
        res2_conv.savefig("Glu78.png")
        res2_conv.close()

        # ### Glu513
        glu513_id = resid2coordid(513, lambda_ref)
        res3_conv = single_residue_convergence(glu513_id, xvg_data_list, lambda_ref)
